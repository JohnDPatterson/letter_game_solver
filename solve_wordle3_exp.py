...
def find_next_word(remaining_words,k_list):
    ...
    # outcome=[]
    # for word in v_words:
    #     w,o=score_word_parallel(word,remaining_words,k_list)
    #     candidate.append(w)
    #     outcome.append(o)
    ...
    with open('word_optimization.txt','w') as f:
        for j in range(len(candidate)):
            f.write(candidate[j]+': ' + str(outcome[j])+ '\n')
    
    candidate=np.delete(candidate,outcome==0)
    outcome=np.delete(outcome,outcome==0)
      
    # idx=np.argpartition(outcome,min([len(outcome),3]))
    idx=np.where(outcome==outcome.min())
    res=candidate[idx]
...
#        print(word)
        ...
        for j in remaining_words:
            ...
            l_list,n,k_list_out,dupe_dict=calc_score(w,list(j[0:-1]),k_list)
            
            possibles=len(calc_remaining_words(l_list,n,k_list_out,dupe_dict,True))   
...
def calc_score(guess_list,possible_list,k_list):
    ...
    k_list_out=copy.deepcopy(k_list)    
    
    for i in range(len(guess_list)):
        n.append([])
        if guess_list[i] not in possible_list:
            l_list.append(guess_list[i])
        elif guess_list[i]==possible_list[i]:
            k_list_out[i]=guess_list[i]
        else:
            n[i].append(guess_list[i])

    all_letters=n+k_list_out
    ...
    return l_list,n,k_list_out,dupe_dict
